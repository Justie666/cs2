...
const WorkoutTerrorists = ({ handleDecreaseBullets, onCoinsUpdate }: IWorkoutTerroristsProps) => {
    const [currentIndex, setCurrentIndex] = useState<number>(0)
    const [isVisible, setIsVisible] = useState<boolean>(false)
    const [terrorists, setTerrorists] = useState<WorkoutTerrorist[]>(workoutTerrorists)
    const [hitPosition, setHitPosition] = useState<{ x: number, y: number } | null>(null)

    const handleHit = (hitType: 'head' | 'body' | 'limbs', x: number, y: number) => {
